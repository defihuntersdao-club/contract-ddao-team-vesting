...
contract DDAOTeamClaim is AccessControl {
    ...
    uint48 public constant TimeStart = 1646092800; // 1 MAR 2022
    uint256 public constant AmountMax = 1680000;
    uint48 public constant TimeEnd = TimeStart + 24 * 86400 * 30;
    ...
    uint48 public immutable TimeDeploy;
    uint256 public immutable BlockDeploy;
    ...
    constructor() {
        ...
        // if (block.chainid == 80001) { // Testnet Mombiy Matik
    ...
    function EpochNext() public onlyAdmin {
        ...
        if (EpochCount == 1) Epoch[EpochCount].time_start = TimeStart;
        else Epoch[EpochCount].time_start = now_time + 1;
        ...
        Epoch[EpochCount].grp_sum =
            Epoch[EpochCount].grp[0] +
            Epoch[EpochCount].grp[1] +
            Epoch[EpochCount].grp[2] +
            Epoch[EpochCount].grp[3];
    ...
    function Claim(
        ...
        require(
            balanceOfToken >= amount_to_send,
            "Not enough balance of DDAO on contract. Contact with administration"
        );
    ...
    function Claimed(address addr) public view returns (uint256 out) {
        if (MemberGrp[addr].enable) out = Payed[MemberGrp[addr].grp][addr];
        else out = 0;
    }

    // !!! REMOVE THE BELOW, TESTS ONLY !!!
    function changeAddrDDAO(address tokenAddr) public onlyAdmin {
        AddrDDAO = tokenAddr;
    }
}
